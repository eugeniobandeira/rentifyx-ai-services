...
- S3 key convention (`assets/{ownerId}/{assetId}/{filename}`) still needs confirmation from the `asset-registry-api` team before `iac/modules/s3-trigger` (still unbuilt) is wired to a real bucket — now tracked on both sides (this repo's `AssetKeyConventionFilter`; `asset-registry-api`'s `STATE.md` gap `G-001`), but still unconfirmed in real code on either side.
