...
Solution builds and CI is green. `Moderation` (E-02, shipped 2026-07-22), `Enrichment` (E-03, shipped 2026-07-23), and `Dedupe` (DEF-AI-001, shipped 2026-07-27 via perceptual hashing — ADR-AI-007) all have real, tested handler stacks. Check `.specs/project/STATE.md` and `.specs/project/ROADMAP.md` for what's actually been implemented before assuming a component exists.
...
All three Lambdas' Terraform IaC exists (`iac/modules/{iam-roles,review-queue,lambda-moderation,lambda-dedupe,s3-trigger,lambda-enrichment,kafka-event-source-mapping}`, composed in `iac/terraform/`), including all idempotency/hash tables and failure DLQs. `terraform validate` is clean end-to-end. Real `apply` is still blocked on `rentifyx-platform` never being applied to AWS and a local credential-plumbing issue (STS `GetCallerIdentity` rejects this session's profile even though the bare AWS CLI call with the same profile works) — see `.specs/project/STATE.md`.
...
- `src/Functions/Dedupe` — S3-triggered, same shape as Moderation. Implemented (DEF-AI-001, ADR-AI-007): `DedupeHandler` (entrypoint) → `DedupeService` (orchestrator) → `AssetKeyConventionFilter` (shared with Moderation, moved to `Shared`) → `DynamoDbIdempotencyStore` (own table, `dedupe-idempotency`) → S3 `GetObjectAsync` → `AverageHashCalculator` (perceptual hash / aHash via `SixLabors.ImageSharp` 3.1.12) → `DynamoDbImageHashStore` (own table, `dedupe-image-hashes`, first-seen-wins via `attribute_not_exists` condition) → `KafkaDedupeEventPublisher` (publishes `AssetDuplicateDetected` only when the hash matches a *different* asset), or the dedupe failure DLQ on any S3/image-decode failure.
...
- `iac/modules/iam-roles` — one dedicated IAM role + policy per Lambda, zero permission overlap between functions (ADR-AI-002). Moderation's policy covers Rekognition, S3 read, DynamoDB PutItem, SQS SendMessage. Enrichment's covers Bedrock InvokeModel (covers Converse too), S3 read, DynamoDB PutItem, SQS SendMessage. Dedupe's covers S3 read, DynamoDB GetItem/PutItem (two tables), SQS SendMessage — the original `rekognition:CompareFaces` placeholder was removed (ADR-AI-007), since asset photos aren't faces.
...
- `iac/modules/{lambda-moderation,lambda-dedupe,s3-trigger,lambda-enrichment,kafka-event-source-mapping}` — all built, composed together with `iam-roles`/`review-queue` in `iac/terraform/` root config (`terraform validate` clean; `apply` still blocked on `rentifyx-platform` never being applied and a local credential-plumbing issue, see `.specs/project/STATE.md`). `iac/modules/s3-trigger` wires both Moderation and Dedupe to the same bucket in a single `aws_s3_bucket_notification` (a bucket supports only one such resource — both Lambdas are separate `lambda_function` blocks inside it, not two resources).
