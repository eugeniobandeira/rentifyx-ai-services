...
Moderation and Enrichment's Terraform IaC exists (`iac/modules/{iam-roles,review-queue,lambda-moderation,s3-trigger,lambda-enrichment,kafka-event-source-mapping}`, composed in `iac/terraform/`), including their idempotency tables and failure DLQs. Dedupe's IAM policy, its two DynamoDB tables (`dedupe-idempotency`, `dedupe-image-hashes`), and its failure DLQ exist too — `iac/modules/lambda-dedupe` (the Lambda function resource + S3 trigger wiring) does not, same "code before IaC" order Moderation and Enrichment both took.
...
- `iac/modules/{lambda-moderation,s3-trigger,lambda-enrichment,kafka-event-source-mapping}` — all built, composed together with `iam-roles`/`review-queue` in `iac/terraform/` root config (`terraform validate` clean; `apply` still blocked on `rentifyx-platform` never being applied and a local credential-plumbing issue, see `.specs/project/STATE.md`). `iac/modules/lambda-dedupe` — not yet built (code shipped ahead of its IaC, same order the other two functions took).
